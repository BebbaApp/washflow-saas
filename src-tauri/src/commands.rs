...
fn first_row(db: &rusqlite::Connection, id: &str, table: &str) -> Result<Value, String> {
...
pub fn upsert_customer(tenant_id: String, id: Option<String>, name: String, phone: Option<String>, email: Option<String>) -> Result<Value, String> {
    ...
    first_row(&db, &record_id, "customers")
}
...
pub fn upsert_service(tenant_id: String, service: Value) -> Result<Value, String> {
    ...
    queue_sync_inner(&db, "services", "INSERT", &id, &json!({"id": id, "tenant_id": tenant_id, "name": name, "price": price}))?;
    first_row(&db, &id, "services")
}

#[tauri::command]
pub fn get_expenses(tenant_id: String) -> Result<Value, String> {
    let db = get_db().lock().map_err(|e| e.to_string())?;
    rows_to_json(&db, "SELECT * FROM expenses WHERE tenant_id = ?1 AND _deleted = 0 ORDER BY date DESC", &[&tenant_id])
}
...
pub fn get_db_info() -> Result<Value, String> {
    ...
    let tables: Vec<String> = {
        ...
        let x: Vec<String> = stmt.query_map([], |r| r.get(0)).map_err(|e| e.to_string())?.filter_map(|r| r.ok()).collect();
        x
    };

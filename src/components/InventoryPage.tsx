...
// ─── Bulk water correction v2 ──────────────────────────────────────────
// After a further 18% reduction to the Usage Guide water values, prior
// consume entries over-deducted by ~18%. Post ONE compensating credit
// equal to 18% of prior consumption (excluding correction entries).
// Guarded so it can only run once.
function WaterCorrectionV2Card({
  waterItemId,
  items,
  transactions,
  canAdjust,
  ...
  const waterTx = transactions.filter((t) => t.itemId === waterItemId);
  const alreadyApplied = waterTx.some((t) => t.source === "Water usage guide correction v2");
  const consumedBefore = waterTx
    .filter(
      (t) =>
        t.type === "consume" &&
        t.source !== "Water usage guide correction" &&
        t.source !== "Water usage guide correction v2",
    )
    .reduce((s, t) => s + Math.abs(t.delta), 0);
  const credit = +(consumedBefore * 0.18).toFixed(3);
    ...
    <div className="glass-card p-4 border-primary/30">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-foreground flex items-center gap-2">
            <RefreshCw className="w-4 h-4 text-primary" />
            Water usage guide correction (v2 · −18%)
          </p>
          <p className="text-[11px] text-muted-foreground mt-1 max-w-xl">
            Usage Guide water values were further reduced by 18%. Historical washes deducted the
            older amounts. Post a single compensating credit of{" "}
            <span className="font-mono text-foreground">+{credit.toFixed(2)} {item.unit}</span>{" "}
            to <span className="font-medium">{item.name}</span> so current stock matches the new guide.
            Past ledger entries are preserved for audit.
          </p>
        </div>
      ...
      <Dialog open={open} onOpenChange={(o) => !busy && setOpen(o)}>
        <DialogContent className="bg-card border-border text-foreground sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Apply water correction (v2)?</DialogTitle>
          </DialogHeader>
          <div className="space-y-2 text-sm">
            <p>
              This will add a single <span className="font-semibold">+{credit.toFixed(2)} {item.unit}</span> adjustment
              to <span className="font-semibold">{item.name}</span>, reflecting the additional 18%
              reduction to the Usage Guide.
            </p>
            <p className="text-xs text-muted-foreground">
              Prior consumption tallied: {consumedBefore.toFixed(2)} {item.unit} · Credit: 18% ={" "}
              {credit.toFixed(2)} {item.unit}
            </p>
          </div>
          <div className="flex gap-2 pt-2">
            ...
            <button
              onClick={async () => {
                ...
                try {
                  await onApply(
                    credit,
                    `Bulk correction v2: usage guide reduced 18% · 18% of ${consumedBefore.toFixed(2)} ${item.unit} credited back`,
                  );
                  setOpen(false);
                } finally {
                  setBusy(false);
